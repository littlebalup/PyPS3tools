...
def printrisklevel(risklevel):
    if risklevel == "WARNING":
        print(colored("yellow", "%s!" % risklevel))
    if risklevel == "DANGER":
        print(colored("red", "%s!" % risklevel))
...
if __name__ == "__main__":
    ...
    for entry in chktree.findall(".//%s/skulistdata/" % flashType):
        filedata = string2hex(
            getDatas(
                rawfiledata,
                int(entry.attrib.get("offset"), 16),
                int(entry.attrib.get("size"), 16),
            )
        )
        tag = entry.text
        if tag == "bootldrsize":
    ...
    for node in chktree.findall(".//%s/skumodels/" % flashType):
        d = {}
        for subnode in chktree.findall(
            ".//%s/skumodels/%s[@id='%s']/"
            % (flashType, node.tag, node.attrib.get("id"))
        ):
    ...
    for sdk in chktree.findall(".//%s/sdk/sdk_version" % flashType):
        index = rawfiledata.find(
            hex2string("73646B5F76657273696F6E").encode("latin"),
            int(sdk.attrib.get("offset"), 16),
            int(sdk.attrib.get("offset"), 16) + 0x4F0,
        )
        addressPos = index - 0xC
        address = int(sdk.attrib.get("offset"), 16) + int(
        ...
        for subnode in chktree.findall(".//%s/%s/" % (flashType, node.tag)):
            ...
            if subnode.tag == "binentry":
                ...
                filedata = string2hex(
                    getDatas(
                        rawfiledata,
                        int(subnode.attrib.get("offset"), 16),
                        len(subnode.text) // 2,
                    )
                )
            ...
            if subnode.tag == "multibinentry":
                ...
                filedata = string2hex(
                    getDatas(
                        rawfiledata,
                        int(subnode.attrib.get("offset"), 16),
                        int(subnode.attrib.get("length"), 16),
                    )
                )
                print("%s :" % subnode.attrib.get("name"), end=" ")
                for entry in chktree.findall(
                    ".//%s/%s/%s[@name='%s']/"
                    % (flashType, node.tag, subnode.tag, subnode.attrib.get("name"))
                ):
                    if filedata.lower() == entry.text.lower():
                        if (
                            subnode.attrib.get("name").endswith("trvk_prg1 SCE")
                            and entry.text == "FFFFFFFF"
                        ):
                            print("Blank")
                            skipHash = True
                        elif (
                            subnode.attrib.get("name").endswith("trvk_pkg1 SCE")
                            and entry.text == "FFFFFFFF"
                        ):
                            print("Blank")
                        else:
                ...
                if subnode.attrib.get("sizeoffset") is not None:
                    size = int(
                        string2hex(
                            getDatas(
                                rawfiledata,
                                int(subnode.attrib.get("sizeoffset"), 16),
                                int(subnode.attrib.get("sizelength"), 16),
                            )
                        ),
                        16,
                    )
                    ...
                    if ChkResult:
                        ...
                        print(
                            ...
                            % (
                                ...
                                int(subnode.attrib.get("offset"), 16) + size,
    ...
    if CID.startswith("0FFF"):
        printcolored("cyan", "This is a refurbished console!")
